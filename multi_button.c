 ...
 * @retval None
 */
void button_attach(struct Button *handle, ButtonEvent event, BtnCallback cb) {
    handle->cb[event] = cb;
}
 ...
 * @retval button event.
 */
ButtonEvent get_button_event(struct Button *handle) { return (ButtonEvent)(handle->event); }
